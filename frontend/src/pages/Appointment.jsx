...
const Appointment = () => {
  ...
  const getAvailableSlot = async (params) => {
    setDocSlots([])
    ...
    for (let  i=0; i < 7; i++){
      ...
      // getting the time of the day with index
      ...
      while (currentDate < endTime) {
        let formatttedTime = currentDate.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <div className="flex gap-3 items-center w-full overflow-x-scroll mt-4">
            {docSlots.length && docSlots.map((item, index)=>(
              <div onClick={()=>setSlotIndex(index)} className={`text-center py-6 mon-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-primary text-white': 'border-gray-400'}`} key={index}>
